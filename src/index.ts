...
const isMainModule =
	...
	(process.argv[1] && process.argv[1].endsWith('mcp-atlassian-jira')) ||
	(process.argv[1] && process.argv[1].endsWith('mcp-server-atlassian-jira')); // Additional binary name pattern check
if (isMainModule) {
	main().catch((err) => {
